...
void GPIO_KEY_Init(void)
{
	  ...
	  PIN_InitConfig(STA1p, PIN_MODE_INPUT, 1);
	  PIN_InitConfig(P33_4, PIN_MODE_INPUT, 1);
}
...
static const GPIO_Name_t key_index[5] = {KEY0p, KEY1p, KEY2p, STA1p, P33_4}; //按键列表
uint8 key_state[5] = {0}; //按键状态
...
uint8 key_switch(void)
{
    ...
    for(i = 0; i < 5; i++)
    {
        if(1 != PIN_Read(key_index[i]))                   // 按键按下
        {
            key_state[i] = 1;
            while(0 == PIN_Read(key_index[i]));
