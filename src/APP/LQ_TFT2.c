...
* @brief TFT2初始化
*
* @param type ：TFT2.0初始化  0：竖屏0度   1:横屏90度  2：倒立1820度  3：270度
*
* @return   无
*
...
void TFT2SPI_Init(unsigned char type)
{
  ...
  if(type==0)// ===== rotate 0 degree ===== //  屏排线与ＩＣ在上方OK
  {
    TFT2mode=0;
    write_command(0x02,0x0100);
    write_command(0x03,0x1030);
...
void TFT2SPI_Write_Cmd(unsigned char cmd)
{
  ...
  for(i=0;i<8;i++)
  {
    TFT2SPI_SCK_L;// SCK=0  ILI9163_SCK=0;
    if(cmd&0x80){TFT2SPI_SDI_H;}// SDI=1
    else{TFT2SPI_SDI_L;}// SDI=0
    TFT2SPI_SCK_H;	// SCK=1  ILI9163_SCK = 1;
    cmd = (cmd<<1);
  }
}
...
void TFT2SPI_Set_Pos(unsigned char xs,unsigned char ys,unsigned char xe,unsigned char ye)
{
    ...
    else if(TFT2mode==2)//  俗称倒装使用
    {
        // =====顺时针 rotate 180 degree =====
        //   屏排线与ＩＣ在上方
        write_command(0x0001, 0x011C);  //0x001C
        write_command(0x0003, 0x1000);
...
void Test_TFT2(void)
{
  TFT2SPI_Init(2);  //LCD初始化  0:横屏  1：竖屏
