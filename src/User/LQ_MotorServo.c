...
#define USEDRV8701  //USEDRV8701 使用龙邱不同的驱动模块，选择对应的宏定义
//电机频率
...
void TestMotor (void)
{
    ...
    MotorInit();
    PIN_InitConfig(DIN, PIN_MODE_OUTPUT, 1);     //刹车线
    DIN_OFF;                        //刹车
    ...
    while (1)
    {
        ...
        if (KEY_Read(START) == 0)      //按下start键 启动或停止动量轮
        {
            if(PIN_Read(DIN))
            {
                DIN_OFF;
...
void ServoInit (void)
{
    ATOM_PWM_InitConfig(ATOMSERVO1, 760, 50);  //舵机频率为100HZ，初始值为1.5ms中值
    ATOM_PWM_InitConfig(ATOMSERVO2, 760, 50);  //舵机理论范围为：0.5ms--2.5ms，大多舵机实际比这个范围小
}
